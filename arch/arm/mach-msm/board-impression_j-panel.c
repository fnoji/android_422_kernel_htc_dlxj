...
static int __init mipi_impression_j_panel_init(void)
{
  int rc;
  ...
  if (panel_type == PANEL_ID_IMN_SHARP_HX)
    {
      rc = mipi_cmd_sharp_init();
      printk(KERN_INFO "match PANEL_ID_IMN_SHARP_HX panel_type\n");
      return rc;
    }
  else if (panel_type == PANEL_ID_IMN_SHARP_NT)
    {
      rc = mipi_video_sharp_nt_720p_pt_init();
      printk(KERN_INFO "match PANEL_ID_IMN_SHARP_NT panel_type\n");
      return rc;
    }
  else
...
late_initcall(mipi_impression_j_panel_init);
